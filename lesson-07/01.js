...
созданными нейросетью изображениями веб-технологий. Изучите файл index.html. В этом задании надо будет работать
с секцией "Урок 6". Разметка уже написано - нужно добавить только js-код.
...
nextButton.addEventListener("click", () => {
    imgIndex++
    if (imgIndex > WEB_TECH_IMAGES.length-1) {
        imgIndex = 0
    }

    imgElement.src = WEB_TECH_IMAGES[imgIndex]
});

prevButton.addEventListener("click", () => {
    imgIndex--
    if (imgIndex < 0) {
        imgIndex = WEB_TECH_IMAGES.length-1
    }
    imgElement.src = WEB_TECH_IMAGES[imgIndex]
});
